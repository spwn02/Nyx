...
struct GpuMaterialPacked {
  ...
  uvec4 tex0123;        // base/emissive/normal/metalRough
  uvec4 tex4_pad;       // ao, pad, pad, pad
...
void main() {
  ...
  vec3 mrTex = sampleLinear(M.tex0123.w, uv);
  float metallic  = clamp(M.mrAoFlags.x * mrTex.b, 0.0, 1.0);
  float roughness = clamp(M.mrAoFlags.y * mrTex.g, 0.02, 1.0);
  float ao        = clamp(M.mrAoFlags.z * sampleLinear(M.tex4_pad.x, uv).r,
                          0.0, 1.0);
  ...
  if (type > LIGHT_SPOT) {
    ...
    vec3 lightCol = Lg.color.rgb * Lg.params.y;
    float shadow = 1.0;
